        ...
        for i in xrange(self.size_of_school):
            ...
            # If the fish has not had a colision with the edge of the tank
                ...
                if not close:
                    ...
                    flocking_v_rect = [0.1 * (alignment[0] + cohesion[0] + separation[0]), 0.1 * (alignment[1] + cohesion[1] + separation[1])]
                    print "rect speed for {0}: x = {1}, y = {1}".format(i, flocking_v_rect[0], flocking_v_rect[1])
                    flocking_v_polar = to_polar(*flocking_v_rect)
                    self.velocities[time,i,1] = flocking_v_polar[1]
                    print "{0} now have theta = {1}".format(i, self.velocities[time,i,1])
